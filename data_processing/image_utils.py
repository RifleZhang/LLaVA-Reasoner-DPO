...
def get_resized_image(image_path, length=MAX_LENGTH):
    img = get_image(image_path)
    width, height = img.width, img.height
    # print(width, height)
    ratio = max(width / length, 1.0)
    ratio = max(height / length, ratio)
    ...
    resized_img.save(img_buffer, format="JPEG")  # You can change JPEG to PNG if you prefer
    img_buffer.seek(0)
