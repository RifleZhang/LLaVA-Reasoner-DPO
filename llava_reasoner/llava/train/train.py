...
from data_processing.utils import load_json_data
...
def maybe_truncate(conversations, truncate_len=None):
    if truncate_len is None:
        return conversations
    for conv in conversations:
        response_len = len(conv['value'].split())
        if response_len > truncate_len:
            logger.info(f"truncate: {conv['value']}")
            conv['value'] = ' '.join(conv['value'].split()[:truncate_len])
    return conversations
...
def preprocess(
    ...
    raise(f"error not implemented preprocess")
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        self.truncate_len = None # TODO: remove hard code, truncate because some data has repeats.
    ...
    def load_and_cache_image(self, image_file, local_folder):
        if './' in image_file:
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0]:
                ...
                image = self.load_and_cache_image(image_file, local_folder=image_folder)
                ...
                sources_ = copy.deepcopy([maybe_truncate(e["conversations"], truncate_len=self.truncate_len) for e in sources])
                if DEFAULT_IMAGE_TOKEN not in sources_[0][0]['value']:
                    sources_[0][0]['value'] = DEFAULT_IMAGE_TOKEN + '\n' + sources_[0][0]['value']
...
def load_data(data_args):
    if len(data_args.data_paths) == 0:
        data_list = load_json_data(data_args.data_path)
    else:
        data_list = []
        sample_ratios = data_args.sample_ratios
        if len(sample_ratios) == 0:
            sample_ratios = [1] * len(data_args.data_paths)
        for data_path, sample_ratio in zip(data_args.data_paths, sample_ratios):
            ds = load_json_data(data_path)
            logger.info(f"loading data from {data_path}, length: {len(ds)}")
            data_list.extend(ds * sample_ratio)
...
def train(attn_implementation="flash_attention_2"):
    ...
    for i in range(min(3, len(training_data))):
        ...
        # assert IMAGE_TOKEN_INDEX in ex_input_ids_0
