...
class DPOTrainer(Trainer):
    ...
    def concatenated_inputs(
        ...
        assert type(batch['images']) == list
        assert type(batch['image_sizes']) == list
        repeated_image_list = batch['images'] * 2
        repeased_image_sizes_list = batch['image_sizes'] * 2
        
        concatenated_batch['concatenated_images'] = repeated_image_list
        concatenated_batch['concatenated_image_sizes'] = repeased_image_sizes_list
    ...
    @staticmethod
    def build_weight(mask, discount_factor_type='none'):
        weight_position = (mask.float().cumsum(dim=1) - 1)
        num_tokens = mask.sum(dim=1)
        if discount_factor_type == 'cos':
            weight = ( 1 + torch.cos( weight_position / num_tokens.unsqueeze(1) * torch.pi) ) / 2
            weight = weight * mask
        elif discount_factor_type == 'linear':
            weight = (num_tokens.unsqueeze(1) - weight_position) / num_tokens.unsqueeze(1)
            weight = weight * mask
        else:
            weight = mask
    ...
    def get_batch_logps(
        ...
        weight = DPOTrainer.build_weight(loss_mask, discount_factor_type)
        if average_log_prob:
            return (per_token_logps * weight).sum(-1) / loss_mask.sum(-1)
        else:
            return (per_token_logps * weight).sum(-1)
    ...
    def concatenated_forward(
        ...
        # images = [torch.tensor(i, dtype=model_dtype) for i in concatenated_batch['concatenated_images']] # use autocast in prediction step
        ...
        all_logps = self.get_batch_logps(
            ...
            is_encoder_decoder=self.is_encoder_decoder,
            label_pad_token_id=self.label_pad_token_id,
            discount_factor_type=self.discount_factor_type,
        )
    ...
    def get_batch_loss_metrics(
        ...
        unscaled_sft_loss = unscaled_sft_loss.detach().cpu()
        ...
        metrics[f"{prefix}losses/sft"] = unscaled_sft_loss
