...
from data_processing.utils import load_json_data, load_and_cache_image
...
class DataArguments:
    ...
    bucket_folder: Optional[str] = field(default=None)
    image_aspect_ratio: str = 'square'
    
    # added 
    num_sample: Optional[int] = field(default=None)
    truncate_len: Optional[float] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    discount_factor_type: str=field(default=None)
...
def preprocess_llama3(
    ...
    return dict(
        input_ids=input_ids[:, :-1], # remove eos special token for dpo alignment
        labels=targets[:, :-1],
    )
    # return dict(
    #     input_ids=input_ids,
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "llama3":
        ...
        raise(f"check template version: {conversation_lib.default_conversation.version}")

def load_data(data_args, mode='train'):
    if mode == 'train':
    ...
    if len(data_paths) == 0:
        ...
        if len(sample_ratios) == 0:
            sample_ratios = [1] * len(data_paths)
        for data_path, sample_ratio in zip(data_paths, sample_ratios):
            data_list.extend(load_json_data(data_path) * sample_ratio)
    logger.info(f"{mode} data list length: {len(data_list)}")
    return data_list
...
class DPODataset(Dataset):
    ...
    def get_item(self, i):
        ...
        if 'image' in data_dict:
            ...
            # hack
            if 'image_data' in image_file:
                image_file = image_file.split('image_data/')[-1]
            image_folder = self.image_folder
            bucket_folder = self.bucket_folder
            image = load_and_cache_image(image_file, bucket_folder=bucket_folder, local_folder=image_folder)
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                image_size = image.size
                image = process_anyres_image(       # torch.Size([5, 3, 336, 336]) # first dim 3 (1x2+1) to 5(2x2+1)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        '''           
        {
            ...
            'chosen': 'A leather crafter is more likely to use these tools. ...',
            'rejected': 'A leather crafter is more likely to use these tools as they consist of a hole punch, scissors, and a knife. ...',
            'image': '/etc/data/vlm_reason/image_data/rlaif/coco2017_train2017_000000489771.jpg',
            'origin_dataset': 'OK-VQA',
            'origin_split': '{"model": "OmniLMM-12B", "feedback_model": "OmniLMM-12B", "type": "question_answering"}',
            'orig_idx': 'OmniLMM-12B_OmniLMM-12B_0',
...
class DPODataCollator(DPODataCollatorWithPadding):
    ...
    def maybe_truncate(self, text: str, max_length, min_len=90) -> str:
        ...
        if max_length > 1:
            ...
            num_words = len(text.split(' '))
            num_words_to_keep = int(max_length * num_words)
            num_words_to_keep = max(num_words_to_keep, min_len)
            return " ".join(text.split(' ')[:num_words_to_keep])
    ...
    def tokenize_batch_element(
        ...
        if self.truncate_len is not None:
            # orig_chosen = chosen
            # orig_rejected = rejected
            chosen = self.maybe_truncate(chosen, max_length=self.truncate_len)
            rejected = self.maybe_truncate(rejected, max_length=self.truncate_len)
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            has_image = True # set to True because all data has image.
        ...
        if 'image' in features[0]:
            ...
            image_sizes = [instance['image_size'] for instance in features]
            # if all(x is not None and x.shape == images[0].shape for x in images):
            #     padded_batch['images'] = torch.stack(images)
            # else:
            padded_batch['images'] = images
            padded_batch['image_sizes'] = image_sizes
        return padded_batch
...
def make_dpo_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = DPODataset(tokenizer=tokenizer,
                                mode='train',
                                data_args=data_args)
    if len(data_args.eval_data_paths) > 0 or data_args.eval_data_path:
    ...
    data_collator = DPODataCollator(
            ...
            truncate_len=data_args.truncate_len,
        )
    return dict(
...
def train(attn_implementation="flash_attention_2"):
    logger.info(f"attn type: {attn_implementation}")
    global local_rank
    ...
    if model_args.tokenizer_name_or_path is None:
        logger.info(f"set tokenizer as model name: {model_args.model_name_or_path}")
    ...
    trainer = LLaVADPOTrainer(
        ...
        discount_factor_type=training_args.discount_factor_type,
